...
Цикл данных (V-P-M-P-V):
1. View сообщает о действии пользователя
2. Presenter сохраняет данные в Model
3. Model валидирует и уведомляет об изменениях  
4. Presenter обновляет View актуальными данными
...
### Класс Card (базовый)
Назначение: Базовый класс для всех карточек товара. Содержит только общий функционал, который используется всеми дочерними классами.
...
### Класс ExtendedCard
Назначение: Промежуточный класс для карточек с категорией и изображением.
Наследует от: Card<IExtendedCard>
...
Наследует от: ExtendedCard<ICardCatalog>
...
Методы:
Нет дополнительных методов - использует функционал родительских классов

### Класс CardPreview
Назначение: Карточка товара в модальном окне просмотра.
Наследует от: ExtendedCard<ICardPreview>
...
set available(value: boolean) - блокирует кнопку если товар недоступен
